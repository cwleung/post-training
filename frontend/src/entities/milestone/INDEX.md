...
| `MilestoneTutorialModal.tsx` | TSX Component | Fullscreen 4-tab interactive workbench with overflow-x containment: 4-Step Pipeline Code with continuous End-to-End full view, unified consolidated Python script card, quick-jump anchors, per-step copy buttons, text wrap toggles (`WrapText`), Architecture Decisions Workbench, Frontier Lab Systems Deep Dive Q&A, and Technical Highlights Generator |
...
- Must ensure modal dialogs never scroll horizontally (`overflow-x-hidden min-w-0 max-w-full`); code blocks scroll horizontally within their dedicated containers and support one-click text wrap toggling.
